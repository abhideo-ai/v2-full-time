...
const move = async (w, id, action, reason, note, until) => {
  li(w.document, id).querySelector(`.dl-act[data-act="${action}"]`)
    .dispatchEvent(new w.MouseEvent("click", { bubbles: true }));
  await settle(40);
  w.document.getElementById("move-reason").value = reason;
...
(async () => {
  ...
  await move(w, "paste-jds", "parked", "posting-closed", "role pulled from their careers page");
  ...
  ok(note.includes("posting-closed") && note.includes("careers page"), `reason + note shown — "${note.slice(0,60)}"`);
  ...
  ok(acts.filter(a => a === "parked").length === 1, "one park in history");
  ok(acts.includes("restored") && acts.includes("dropped"), "drop AND its restore both kept");
  ok(h.find(e => e.action === "parked").reason === "posting-closed", "reason persisted to history");
  ...
  if (fs.readFileSync(REPO + "/daily/index.html", "utf8").includes('data-day="2026-08-24"')) {
    await fetch(API + "/api/ops", { method: "POST", body: JSON.stringify({ ops: [
      ...
      { key: "2026-08-24::old-parked", action: "parked", reason: "posting-closed" },
    ]})});
    w = await boot(API); d = w.document;
